...
max_size = op.cameras[0].config.image.resolution.width
        ...
        if n_sample % display_rate == 0:
            ...
            for i in range(len(images)):
                ...
                if detector_activated:
                    ...
                    detection_weapons = detector.detect_weapons(images[i], max_size) # prediction weapons
                    #draw bounding box in the frame
                    images[i] = bounding_box(images[i], detections=detection_weapons, class_names=detector.class_names, infer_conf=detector.weapons_detector.conf)

                    detection_people = detector.detect_people(images[i], max_size, recording) # prediction people

                    if not recording:
                    # draw bounding box in the frame
                        images[i] = bounding_box(images[i], detections=detection_people, class_names=detector.class_names, infer_conf=detector.people_detector.conf)
